...
defender.StartMenu.prototype = {
	...
	create: function () {             
		...
		startPrompt = this.add.bitmapText(this.world.centerX-155, this.world.centerY, 'eightbitwonder', 'CLICK TO START', 25);
		startPrompt = this.add.bitmapText(this.world.centerX-215, this.world.centerY+90, 'eightbitwonder', 'Use the following buttons', 18);
		startPrompt = this.add.bitmapText(this.world.centerX-215, this.world.centerY+120, 'eightbitwonder', 'to open fire:', 18);
		startPrompt = this.add.bitmapText(this.world.centerX-185, this.world.centerY+160, 'eightbitwonder', 'space bar:', 18);
		startPrompt = this.add.bitmapText(this.world.centerX-185, this.world.centerY+190, 'eightbitwonder', 'left mouse button:', 18);
	},
...
defender.Game.prototype = {    
	...
	this.buildBullet();
        this.buildEmitter();
        this.countdown = this.add.bitmapText(10, 10, 'eightbitwonder', victim+' left ' + this.totalCreatures, 20);
        this.timer.start();
	this.timer2.start();
    },  
	...
	for(var i=0; i<2; i++) 
		{
	var p = this.stingray.create(this.rnd.integerInRange(200, this.world.width-50), this.rnd.integerInRange(this.world.height-175, this.world.height-60), 'seacreatures', 'stingray0000');
            p.anchor.setTo(0.5, 0.5);
	     p.scale.x=0.4;p.scale.y=0.4;			
            p.body.moves = false;
            p.animations.add('swim', this.game.math.numberArray(174,197));
            p.animations.play('swim', 24, true);
t = this.add.tween(p).to({ x: -10 }, this.rnd.integerInRange(13000, 22000), Phaser.Easing.Quadratic.InOut, true, 0);	
t.onComplete.add(this.checking, this);
	...
	for(var i=0; i<2; i++) 
		{
	var p = this.crab.create(this.rnd.integerInRange(100, this.world.width-50), this.world.height-55, 'seacreatures', 'crab10000');
            p.anchor.setTo(0.5, 0.5);
			p.scale.x=0.7;p.scale.y=0.7;
			p.body.moves = false;
            p.animations.add('swim', this.game.math.numberArray(33,58));
            p.animations.play('swim', 26, true); 
this.add.tween(p).to({ x: this.world.width-20 }, this.rnd.integerInRange(13000, 18000) , Phaser.Easing.Quadratic.InOut, true, 0, 1000, true);			
	}
	...
	this.blueJellyfish = this.add.group();
        ...
        for(var i=0; i<9; i++) 
		{
            var p = this.blueJellyfish.create(this.rnd.integerInRange(15, this.world.width-50), this.rnd.integerInRange(this.world.height-165, this.world.height-60), 'seacreatures', 'blueJellyfish0000');
            p.anchor.setTo(0.4, 0.4);
			p.scale.x=0.9;p.scale.y=0.9;
			p.body.moves = false;
            p.animations.add('swim', this.game.math.numberArray(0,32));
            p.animations.play('swim', 33, true); 
 this.add.tween(p).to({ y: this.world.height-165 }, this.rnd.integerInRange(8000, 11000), Phaser.Easing.Quadratic.InOut, true, 0, 1000, true);			
        }
	...
	this.purpleFish = this.add.group();
        ...
        for(var i=0; i<2; i++) 
		{
            var p = this.purpleFish.create(this.rnd.integerInRange(this.world.width+5, this.world.width+20), this.rnd.integerInRange(this.world.height-170, this.world.height-60), 'seacreatures', 'purpleFish0000');
            p.anchor.setTo(0.4, 0.4);
			p.scale.x=0.8;p.scale.y=0.8;			
            p.body.moves = false;
            p.animations.add('swim', this.game.math.numberArray(134,140));
            p.animations.play('swim', 8, true); 
	     this.add.tween(p).to({ x: -40 }, this.rnd.integerInRange(13000, 21000), Phaser.Easing.Quadratic.InOut, true, 0, 1000, false);
	}	
	},
	...
	{
	p.scale.x=-0.5;
	t = this.add.tween(p).to({ x: this.world.width }, this.rnd.integerInRange(14000, 21000), Phaser.Easing.Quadratic.InOut, true, 0);	
	}
	else
	{
	p.scale.x=0.5;
	t = this.add.tween(p).to({ x: -10 }, this.rnd.integerInRange(14000, 21000), Phaser.Easing.Quadratic.InOut, true, 0);	
	}
t.onComplete.add(this.checking, this);
	},
       ...
       buildPandas: function() {
		...
		{
            var p = this.pandaGroup.create(this.rnd.integerInRange(5, this.world.width-50), this.rnd.integerInRange(this.world.height-165, this.world.height-60), 'panda', '5904788059a0d793134691_000');
            p.anchor.setTo(0.5, 0.5);
			p.bringToTop();
            p.body.moves = false;
    ...
    buildAliens1: function() {
        ...
        for(var i=0; i<this.totalAliens1; i++) {
           var r = this.aliens1group.create(this.rnd.integerInRange(0, this.world.width-50), this.rnd.realInRange(-1500, 0), 'alien1');
...
var scale = this.rnd.realInRange(0.7, 1);
	...
	buildAliens2: function() {
        ...
        for(var i=0; i<this.totalAliens2; i++) {
           var r = this.aliens2group.create(this.rnd.integerInRange(0, this.world.width-50), this.rnd.realInRange(-1500, 0), 'alien2');
 r.anchor.setTo(0.5, 0.5);			
   var scale = this.rnd.realInRange(0.7, 1);
   r.lives=2;
            r.scale.x = scale;
...
b.kill();		
    ...
    checkCreaturesLeft: function() {
        if(this.totalCreatures <= 0){
            ...
            this.overmessage = this.add.bitmapText(this.world.centerX-250, this.world.centerY-40, 'eightbitwonder', 'GAME OVER\n\n' + this.secondsElapsed + " sec elapsed", 38);
	     ...
	     this.button = this.add.button(this.world.centerX-200, this.world.centerY+110, 'home', this.toTheBeginning, this, 1, 0, 2);
	     this.button2 = this.add.button(this.world.centerX, this.world.centerY+110, 'restart', this.quitGame, this, 1, 0, 2);
   ...
   if(r.y > this.world.height) {
	...
	var r = this.alien_queen.create(this.world.centerX, this.rnd.realInRange(-1500, 0), 'alien_queen');
 r.anchor.setTo(0.5, 0.5);
r.lives=5; 
            this.physics.enable(r, Phaser.Physics.ARCADE);
            r.enableBody = true;
	...
	this.physics.arcade.overlap(this.aliens2group, this.blueJellyfish, this.collision, null, this);
	this.physics.arcade.overlap(this.aliens1group, this.blueJellyfish, this.collision, null, this);
	...
	this.physics.arcade.overlap(this.aliens1group, this.bullets, this.bulletsCollision, null, this);
    this.physics.arcade.overlap(this.aliens2group, this.bullets, this.bulletsCollision, null, this);
	this.physics.arcade.overlap(this.alien_queen, this.bullets, this.bulletsCollision2, null, this);
